...
const unsigned int uiMAX_REG_X = 80;	  /* max. # contextual regions in x-direction */
const unsigned int uiMAX_REG_Y = 60;	  /* max. # contextual regions in y-direction */
...
int iMinClipLimit = int(ceil(float(ulNrPixels)/uiNrBins));
//there is no round in math.h => add 0.5 and call floor instead (or substract 0.5 and call ceil)
//ulClipLimit = unsigned long(iMinClipLimit + round((fCliplimit*(ulNrPixels-iMinClipLimit))));
ulClipLimit = unsigned long(iMinClipLimit + floor((fCliplimit*(ulNrPixels-iMinClipLimit))+0.5));
...
//    ulBinIncr = ulNrExcess / uiNrGreylevels;		  /* average binincrement */
    ulBinIncr = unsigned long(floor(float(ulNrExcess)/ uiNrGreylevels));		  /* average binincrement */
...
//		there is no max in math.h
//		ulStepSize = max(int(floor(uiNrGreylevels/float(ulNrExcess))),1);
		ulStepSize = int(floor(uiNrGreylevels/float(ulNrExcess)));
